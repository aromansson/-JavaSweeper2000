...
public class JavaSweeper extends JFrame { // главный класс
	...
	private final int COLS = 15; // сколько у нас будет столбцов
	private final int ROWS = 15; // сколько будет строк
	private final int BOMBS = 100; // сколько будет бомб
	...
	private String getMessage() { // метод, определяющий, какой текст отображать на метке
		...
		case PLAYED:
			return "Смотри не обосрись, осталось " + game.getBombsRemain() + " бомб!";
		case BOMBED:
			return "ОБОСРАЛСЯ! Хватило на " + game.getTime() + " секунд)";
		case WINNER:
			return "Ладно, на этот раз не обосрался! Нашел всё за " + game.getTime() + " секунд";
		default:
			return null;
		}
	}
